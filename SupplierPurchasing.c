...
#define CASH    1000    //  cash you have to start with.
...
typedef struct _item {
    ...
    unsigned cost;      //  what the item costs to buy.
...
int main(void){
    unsigned i, j;
    unsigned best_item;
    unsigned remaining_space, remaining_cash;
    ...
    for (i=0; i<N; i++){
        ...
        //  display items.
        if(SHOWLIST){
            profit_density = (((1.0 - items[i].b) / items[i].m) - (float)items[i].cost) / (float)items[i].size;
            printf("%u\t%u\t\t%u\t%u\t%.2f\t%.2f\t", i, items[i].quantity, items[i].size, items[i].cost, items[i].m, items[i].b);
    ...
    do{
        ...
        for (i = 0; i < N; i++){
                ...
                profit_density = ((((1.0 + (float)items[i].bought) - items[i].b) / items[i].m) - (float)items[i].cost) / (float)items[i].size;
    ...
    for(i=0; i<N; i++){
        if(items[i].bought){
            ...
            profit_made += ((float)items[i].bought)*((float)items[i].bought - items[i].b) / items[i].m - (float)items[i].bought*(float)items[i].cost;
            spent += (float)items[i].bought * (float)items[i].cost;
        }
    }
